...
BAKED_DIR=$SCRIPT_LOCATION/../../baked/$TARGET_PREFIX$OSMAND_TARGET$CPU_SPECIFIC_SUFFIX$BUILD_TYPE_SUFFIX.makefile
rm -rf "$BAKED_DIR"
mkdir -p "$BAKED_DIR"
(cd "$BAKED_DIR" && cmake -G "Unix Makefiles" $OSMAND_TARGET_TOOLCHAIN $CMAKE_BUILD_TYPE $OSMAND_CPU_SPECIFIC_DEFINE "$WORK_ROOT")
