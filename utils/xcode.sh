...
BAKED_DIR=$SCRIPT_LOCATION/../../../baked/$OSMAND_TARGET_PREFIX-$OSMAND_TARGET$CPU_SPECIFIC_SUFFIX$BUILD_TYPE_SUFFIX
rm -rf "$BAKED_DIR"
mkdir -p "$BAKED_DIR"
(cd "$BAKED_DIR" && cmake -G "Xcode" $OSMAND_TARGET_TOOLCHAIN $CMAKE_BUILD_TYPE $OSMAND_CPU_SPECIFIC_DEFINE \
	-DCMAKE_XCODE_ATTRIBUTE_GCC_VERSION:STRING="com.apple.compilers.llvm.clang.1_0" \
	-DCMAKE_XCODE_ATTRIBUTE_CLANG_CXX_LANGUAGE_STANDARD:STRING="c++0x" \
	-DCMAKE_XCODE_ATTRIBUTE_CLANG_CXX_LIBRARY:STRING="libc++" "$WORK_ROOT")
