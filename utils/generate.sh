...
BAKED_DIR="$SCRIPT_LOCATION/../../baked/$TARGET_PREFIX$OSMAND_TARGET$CPU_SPECIFIC_SUFFIX$BUILD_TYPE_SUFFIX.$TARGET_BUILD_TOOL_SUFFIX"
rm -rf "$BAKED_DIR"
mkdir -p "$BAKED_DIR"
(cd "$BAKED_DIR" && cmake -G "$CMAKE_GENERATOR" -DCMAKE_TARGET_BUILD_TOOL:STRING=$TARGET_BUILD_TOOL_SUFFIX $OSMAND_TARGET_TOOLCHAIN $CMAKE_BUILD_TYPE $OSMAND_CPU_SPECIFIC_DEFINE "$WORK_ROOT")
